...
class TestMap(unittest.TestCase):
    ...
    def test_getitem_bad_index(self):
        map = Map('../maps/test-map1.txt')
        self.assertRaises(IndexError, map.__getitem__, Position(map.height, map.width - 1))
        self.assertRaises(IndexError, map.__getitem__, Position(map.height-1, map.width))
        self.assertRaises(IndexError, map.__getitem__, Position(100, 100))
    ...
    def test_dist_between(self):
        map = Map('../maps/test-map1.txt')
        self.assertEqual(1, map.dist_between(map[Position(0, 0)], map[Position(0, 1)]))
        self.assertEqual(15, map.dist_between(map[Position(0, 0)], map[Position(5, 10)]))
        self.assertEqual(15, map.dist_between(map[Position(5, 10)], map[Position(0, 0)]))
        self.assertEqual(14, map.dist_between(map[Position(1, 0)], map[Position(5, 10)]))
